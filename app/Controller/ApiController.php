...
class ApiController extends AppController
{
    ...
    public function followUser()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (count($follower_details) < 1) {
                ...
                if ($sender_id != $receiver_id && strlen($receiver_details['User']['device_token']) > 8) {
	...
	$this->loadModel("VideoComment");
	    ...
	    if ($section == 0) {
                $hashtags = $this->HashtagVideo->getHashtagsWhichHasGreaterNoOfVideos($starting_point);
                if(count($hashtags) > 0) {
	        ...
	        $users = $this->Video->getUsersWhichHaveGreaterNoOfVideos($starting_point, $section);
		if (count($users) > 0) {
    ...
    public function addTopicFavourite()
    {
        $this->loadModel("TopicFavourite");
        ...
        if ($this->request->isPost()) {
            ...
            $topic_fav['user_id'] = $user_id;
            $topic_fav['created'] = $created;

            $details = $this->TopicFavourite->ifExist($topic_fav);

            if(count($details) > 0){
                $this->TopicFavourite->id = $details['TopicFavourite']['id'];
                $this->TopicFavourite->delete();
                $msg = "unfavourite";
            } else{
                $this->TopicFavourite->save($topic_fav);

                $id = $this->TopicFavourite->getInsertID();
                $details = $this->TopicFavourite->getDetails($id);
    ...
    public function showFavouriteHashtags()
    {
        ...
        if ($this->request->isPost()) {
		    ...
		    $fav_posts[$key]['Hashtag']['favourite'] = 1;
                }
                $output['code'] = 200;
...
//pr($comments);
    ...
    public function showVideoDetail(){
	    ...
	    $video_detail['Topic'] = $this->Video->getTopic($video_detail['HashtagVideo']
);
    ...
    public function showVideosAgainstTopic(){
        ...
        if ($this->request->isPost()) {
            ...
            $topic_details = $this->Topic->ifExist($topic);

            if(count($topic_details) > 0) {
                $videos = $this->TopicVideo->getTopicVideosWithLimit($topic_details['Topic']['id'],$starting_point);

                $topic_views = $this->TopicVideo->countTopicViews($topic_details['Topic']['id']);
            ...
            if(count($videos) > 0) {
                foreach($videos as $key=>$video){
                    ...
                    if($user_id > 0) {
                        $video_data['user_id'] = $user_id;
                        $video_data['video_id'] = $video['Video']['id'];
                        $video_data['topic_id'] = $topic_details['Topic']['id'];
                        $video_like_detail = $this->VideoLike->ifExist($video_data);
                        $video_favourite_detail = $this->VideoFavourite->ifExist($video_data);
                        $topic_favourite_detail = $this->TopicFavourite->ifExist($video_data);
                        ...
                        if (count($topic_favourite_detail) > 0) {

                            $videos[$key]['Topic']['favourite'] = 1;

                        } else {

                            $videos[$key]['Topic']['favourite'] = 0;
                        }
                ...
                $topic_videos_count = $this->TopicVideo->countTopicVideos($topic_details['Topic']['id']);

                $output['code'] = 200;
                $output['msg'] = $videos;
                $output['views'] = $topic_views[0]['total_sum'];
                $output['videos_count'] = $topic_videos_count;
	...
	$this->loadModel('Follower');
        ...
        if ($this->request->isPost()) {
            ...
            $videos = $this->Video->getVideoReplies($user_id,$video_id,$starting_point);
...
*/
    ...
    public function addPhoneDeviceData($device_token,$ip,$device,$version)
    {
...
Please enter this verification code to reset your email.<br><br>Confirmation code: <b></b>" . $code . "<b>";
    ...
    public function showFeaturedHashtags(){
      $this->loadModel('Hashtag');
      if ($this->request->isPost()) {
        $json = file_get_contents('php://input');
        $data = json_decode($json, TRUE);

        $hashtags = $this->Hashtag->getFeaturedHashtags();
