...
class Regular
{
    ...
    function videoToGif($original_video_file_path, $user_id) {
        ...
        $gif = "ffmpeg -ss 1 -t 0.5 -i $original_video_file_path -vf 'fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse' -loop 0 $genrateGifPath";
